...
def discriminator(seed=None):
    ...
    act1 = nn.LeakyReLU()
    linear2 = nn.Linear(in_features=256, out_features=256)
    act2 = nn.LeakyReLU()
    linear3 = nn.Linear(in_features=256, out_features=1)
    model = nn.Sequential(
        flatten_layer,
        linear1,
        act1,
...
def generator(noise_dim=NOISE_DIM, seed=None):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    linear1 = nn.Linear(in_features=noise_dim, out_features=1024)
    linear2 = nn.Linear(in_features=1024, out_features=1024)
    linear3 = nn.Linear(in_features=1024, out_features=784)
    model = nn.Sequential(
        linear1,
        nn.ReLU(),
        linear2,
...
def discriminator_loss(logits_real, logits_fake):
    ...
    fake_label = torch.zeros(size=(N_fake,)).type(dtype)
    N_real = logits_real.size(0)
    real_label = torch.ones(size=(N_real,)).type(dtype)
...
def generator_loss(logits_fake):
    ...
    N_fake = logits_fake.size(0)
    fake_label = torch.ones(size=(N_fake,)).type(dtype)
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = 1/2 * torch.mean((scores_real-1)**2)+1/2*torch.mean(scores_fake**2)
...
def build_dc_generator(noise_dim=NOISE_DIM):
    ...
    batch1 = nn.BatchNorm1d(1024)
    fc2 = nn.Linear(1024,7*7*128)
    batch2 = nn.BatchNorm1d(7*7*128)
    conv1 = nn.ConvTranspose2d(128, 64, kernel_size=4, stride=2, padding=1)
    batch3 = nn.BatchNorm2d(64)
    conv2 = nn.ConvTranspose2d(64, 1, kernel_size=4, stride=2, padding=1)
    model = nn.Sequential(
        fc1, 
        nn.ReLU(),
        batch1,
